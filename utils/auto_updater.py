...
def _normalize(version: str) -> tuple:
    """'2026.07.04' and '2026.7.4' must compare equal — pip strips leading
    zeros from each dot-separated segment on install, PyPI's raw metadata
    doesn't. Comparing as strings caused a false "new version" on every
    check, which triggered an endless upgrade+restart loop."""
    try:
        return tuple(int(part) for part in version.split("."))
    except ValueError:
        return (version,)  # fallback: treat as opaque, only exact match counts
...
async def check_and_update(client=None) -> bool:
    ...
    try:
        if _normalize(latest) <= _normalize(current):
            return False  # already current (covers "2026.07.04" == "2026.7.4" too)
    except TypeError:
        # one of the versions had a non-numeric suffix we couldn't compare —
        # fall back to a plain equality check rather than looping forever
        if latest == current:
            return False
